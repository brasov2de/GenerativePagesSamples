...
// Modified theme loader to return both light and dark themes
async function loadThemesFromXml(): Promise<{ light: ReturnType<typeof createLightTheme>, dark: ReturnType<typeof createDarkTheme> }> {
...
// Statistics Gallery component (unchanged)
interface StatisticsGalleryProps {
  month: Date;
  dailyTotals: Record<string, number>;
  onDayClick: (date: Date) => void;
}
const StatisticsGallery: React.FC<StatisticsGalleryProps> = ({ month, dailyTotals, onDayClick }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
      {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((wd, i) => (
        <div key={`h-${i}`} style={{ textAlign: 'center', fontWeight: 600, marginBottom: 4 }}>{wd}</div>
      ))}
      {cells.map((date, i) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 12, color: tokens.colorNeutralForeground3 }}>{total || '-'}</div>
          </div>
        );
      })}
    </div>
...
const GeneratedComponent: React.FC<GeneratedComponentProps> = ({ dataApi }) => {
  ...
  const fetchMonthlyStats = useCallback(async () => {
    ...
    const filter = `(Microsoft.Dynamics.CRM.EqualUserId(PropertyName='ownerid')) and diana_date ge ${monthStart.toISOString()} and diana_date le ${monthEnd.toISOString()}`;
    const result = await dataApi.queryTable("diana_timeentry", {
      select: ["diana_date", "diana_value"],
      filter
    });
    ...
    result.rows.forEach(r => {
      if (!r.diana_date) return;
      const dateKey = new Date(r.diana_date as any).toISOString().substring(0, 10);
      totals[dateKey] = (totals[dateKey] || 0) + (r.diana_value || 0);
    });
    setMonthlyStats(totals);
  }, [dataApi, selectedDate]);

  useEffect(() => {
    loadThemesFromXml().then(setThemes);
    fetchLookups();
  }, [fetchLookups]);
  ...
  const handleStatDayClick = (date: Date) => {
  ...
  return (
    <FluentProvider theme={isDarkMode ? themes.dark : themes.light} style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
      {/* Header */}
      <div style={{ height: 40, display: 'flex', alignItems: 'center', padding: '0 8px', borderBottom: `1px solid ${tokens.colorNeutralStroke1}`, justifyContent: 'space-between' }}>
        <strong>Time Tracking</strong>
        <Button appearance="secondary" onClick={() => setIsDarkMode(m => !m)}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* Side stats panel */}
        <div style={{ width: statsOpen ? 250 : 32, borderLeft: `1px solid ${tokens.colorNeutralStroke1}`, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          <div style={{ padding: 4, cursor: 'pointer' }} onClick={() => setStatsOpen(o => !o)}>
            {statsOpen ? <PanelRightExpandRegular /> : <PanelLeftExpandRegular />}
          </div>
          {statsOpen && (
            <div style={{ padding: 8 }}>
              <StatisticsGallery month={selectedDate} dailyTotals={monthlyStats} onDayClick={handleStatDayClick} />
            </div>
          )}
        </div>
      </div>
